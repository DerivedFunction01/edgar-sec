...
- **Snapshot Lifecycle**: Incremental publication may inherit immutable payload parts for late CIK augmentation. Finalized SQLite partitions are streamed in bounded batches; DuckDB handles spillable Parquet joins/deduplication under machine-local resource limits, while parallel `vacuum` materializes selected snapshots by bounded quarter workers, validates dependency closure, and atomically advances the current pointer with progress events.
